...
data class AtlasOptimizationConfig(
     ...
     */
    fun shouldUseLowQuality(zoomLevel: Float): Boolean = zoomLevel <= lowQualityZoomThreshold

    /**
     * Gets the appropriate atlas size for the given zoom level
     */
    fun getAtlasSize(zoomLevel: Float, deviceMaxSize: IntSize): IntSize = if (shouldUseLowQuality(zoomLevel)) {
     ...
     */
    fun getBitmapConfig(zoomLevel: Float): Bitmap.Config = if (shouldUseLowQuality(zoomLevel)) {
