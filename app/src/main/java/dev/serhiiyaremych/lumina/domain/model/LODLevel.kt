...
enum class LODLevel(
    ...
    companion object {
         ...
         */
        fun forZoom(zoom: Float): LODLevel = values().firstOrNull { zoom in it.zoomRange } ?: LEVEL_7

        /**
         * Get LOD level by numeric level value
         */
        fun fromLevel(level: Int): LODLevel? = values().firstOrNull { it.level == level }

        /**
         * Get all LOD levels in order of increasing quality
         */
        fun getAllLevels(): List<LODLevel> = values().sortedBy { it.level }
